...
///
/// Checks `menus.overflow_thread` and renders nothing when the menu is closed.
...
pub fn OverflowMenu() -> Element {
    ...
    let Some(thread_id_str) = state.menus.overflow_thread.clone() else {
        return rsx! {};
    };
    let position = state.menus.overflow_position;
    let sender_str = state.menus.thread_sender.clone().unwrap_or_default();
