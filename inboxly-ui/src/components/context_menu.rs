...
///
/// Checks `menus.context_thread` and renders nothing when the menu is closed.
...
pub fn ContextMenu() -> Element {
    ...
    let Some(thread_id_str) = state.menus.context_thread.clone() else {
        return rsx! {};
    };
    let position = state.menus.context_position;
